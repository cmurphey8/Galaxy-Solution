...
public class NBody {
    ...
    public static void loadFile() {
        ...
        object[0] = new BlackHole(scan, R);     // assumes first object is supermassive black hole
        for (int i = 1; i < N; i++) {
            object[i] = new Star(scan, R);      // assumes all other objects begin as stars
        }
    }
        ...
        for (double t = 0.0; t < T; t += dt) {
            ...
            for (int i = 0; i < N; i++) {
                if (!object[i].isRemoved()) {
                    ...
                    object[i].zeroF();
                    for (int j = 0; j < N; j++) {
                        if (i != j && !object[j].isRemoved()) {
                            object[i].updateF(object[j], G);
                        }    
                    }
